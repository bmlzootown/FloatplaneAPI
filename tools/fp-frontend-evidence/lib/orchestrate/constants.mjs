/** Phase 2.3 — wire extract + compare into cumulative frontend-observation workflow. */

export const ORCHESTRATOR_ID = 'phase2.3-frontend-evidence-orchestrate';
export const ORCHESTRATOR_VERSION = '2.3.0';
...
/**
 * Commit strategy (documented for §7):
 * Prefer combined Extract+Compare when both produce new artifacts in one run
 * (fewer FF push races). Never combine with Phase 1 Observe in one commit.
 */
export const COMMIT_STRATEGY = Object.freeze({
  ...
  rationale:
    ...
    '(race-safer than two FF pushes). Extract-only or Compare-only when the other ' +
    'step is not yet possible.',
});
